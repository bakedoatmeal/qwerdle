...
const Board = ({guesses, wordSize, wordStatuses, letterStatus, keyword}) => {

  const allGuesses = guesses.map((word, index) => {
    const copyLetterStatus = {...letterStatus};
    if (wordStatuses[index] === 0) {
      return (
        <Word size={wordSize} guess={word} key={index} wordStatus={wordStatuses[index]} letterStatus={letterStatus} keyword={keyword}/>
      )
    } else {
      return (
